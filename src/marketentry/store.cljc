(ns marketentry.store
  "SSoT for the HND market-entry compliance actor, behind a `Store`
  ...
  Each engagement also carries `:registering-camara` and
  `:business-domicile-department` -- the ground truth
  `marketentry.governor`'s flagship Cámara jurisdiction check
  independently verifies. This is DOMICILE-CONDITIONAL: only a
  chamber/department pair that does NOT match this catalog's
  independently-confirmed `camara-jurisdiction-departments` map is a
  violation, never a blanket rule against any particular chamber.
...
(defn demo-data
  ...
  eng-4 vs eng-5 is the flagship contrasting pair: eng-4 claims a
  registering camara (CCIT, Francisco Morazán) that does NOT cover its
  own declared domicile department (Cortés) -> HARD hold. eng-5 is a
  San Pedro Sula company correctly claiming CCIC (Cortés) for a
  Cortés domicile -> clean, proving the check does not false-positive
  on a legitimate match."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Consultores Morazán S. de R.L." :portal "HonduCompras"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :registering-camara "CCIT" :business-domicile-department "Francisco Morazán"
             :drafted? false :submitted? false
             :jurisdiction "HND" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "HonduCompras"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :registering-camara "CCIT" :business-domicile-department "Francisco Morazán"
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Servicios Tegucigalpa Ltda." :portal "HonduCompras"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 999000.0
             :registering-camara "CCIT" :business-domicile-department "Francisco Morazán"
             :drafted? false :submitted? false
             :jurisdiction "HND" :status :intake}
    "eng-4" {:id "eng-4" :operator "Puerto Cortés Import S.A." :portal "HonduCompras"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :registering-camara "CCIT" :business-domicile-department "Cortés"
             :drafted? false :submitted? false
             :jurisdiction "HND" :status :intake}
    "eng-5" {:id "eng-5" :operator "San Pedro Sula Logistics S. de R.L." :portal "HonduCompras"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :registering-camara "CCIC" :business-domicile-department "Cortés"
             :drafted? false :submitted? false
             :jurisdiction "HND" :status :intake}
    "eng-6" {:id "eng-6" :operator "Obras Públicas del Valle" :portal "HonduCompras"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               registering-camara business-domicile-department
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    registering-camara                    (assoc :engagement/registering-camara registering-camara)
    business-domicile-department          (assoc :engagement/business-domicile-department business-domicile-department)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/registering-camara :engagement/business-domicile-department
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :registering-camara (:engagement/registering-camara m)
     :business-domicile-department (:engagement/business-domicile-department m)
