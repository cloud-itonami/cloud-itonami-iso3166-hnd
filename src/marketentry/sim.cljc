(ns marketentry.sim
  ...
  Cámara de Comercio territorial-jurisdiction mismatch and its
  domicile-conditional non-trigger for a matching chamber/department
  pair."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (HND, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up camara-jurisdiction-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (CCIT claimed for a Cortés domicile -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (CCIC for Cortés -- correct match, must NOT fire) ==")
