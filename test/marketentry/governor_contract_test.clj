  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Consultores Morazán S. de R.L."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Consultores Morazán S. de R.L." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest camara-jurisdiction-mismatch-is-held-and-unoverridable
  (testing "CCIT (Francisco Morazán) claimed for a Cortés domicile -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:camara-jurisdiction-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest camara-jurisdiction-mismatch-does-not-fire-for-a-correct-match
  (testing "DOMICILE-CONDITIONAL: CCIC correctly claimed for a Cortés domicile must NOT trigger the flagship check"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-5")
          _ (draft! actor "t5bpre" "eng-5")]
      (is (= "CCIC" (:registering-camara (store/engagement db "eng-5"))) "fixture really claims CCIC")
      (is (= "Cortés" (:business-domicile-department (store/engagement db "eng-5"))) "fixture really is domiciled in Cortés")
      (let [r1 (exec-op actor "t5b" {:op :filing/submit :subject "eng-5"} operator)]
        ...
        (let [r2 (approve! actor "t5b")]
          (is (= :commit (get-in r2 [:state :disposition])))
          (is (not (some #{:camara-jurisdiction-mismatch} (-> (store/ledger db) last :basis))))
