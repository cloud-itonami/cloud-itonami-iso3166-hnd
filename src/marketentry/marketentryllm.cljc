(ns marketentry.marketentryllm
  ...
  the HND public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real HonduCompras portal
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world HonduCompras portal
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "registering-camara=" (:registering-camara e)
                        " business-domicile-department=" (:business-domicile-department e)
     ...
     :confidence (if (and e
                          (= (:registering-camara e)
                             (case (:business-domicile-department e)
                               "Francisco Morazán" "CCIT"
                               "Cortés" "CCIC"
                               ::unknown)))
                   0.9 0.3)}))
